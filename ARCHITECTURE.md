...
| 3 | **ntfy topic URL** (acts as bearer secret — anyone with URL can publish/read) | Self-hosted: `http://192.168.1.5:8080/nastemp`. Public: `https://ntfy.sh/my-secret-topic-xyz` (pick unguessable suffix). No signup. Test: `curl -d "test" <URL>`. Sent on: 1st API fail (`on_api_fail`), failsafe (`on_failsafe`), step_up, critical. | `config.yaml: ntfy.url`, `ntfy.enabled`, `on_step_up/on_step_down/on_failsafe/on_critical/on_api_fail`. Watchdog (Docker): optional `NASTEMP_NTFY` env. | `fan_controller.py:ntfy()`, `watchdog.py` urllib block |
